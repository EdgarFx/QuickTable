...
class Detector(object):
    ...
    def save_coco_results(self, image_list, results, use_coco_category=False):
        ...
        if bbox_results:
            if not os.path.exists(self.output_dir+os.path.split(image_list[0])[-1][:-4]+'/'):
                os.makedirs(self.output_dir+os.path.split(image_list[0])[-1][:-4]+'/')
            bbox_file = os.path.join(self.output_dir, os.path.split(image_list[0])[-1][:-4], "%s.json"%os.path.split(image_list[0])[-1][:-4])
...
def visualize(image_list, result, labels, output_dir='output/', threshold=0.5):
    ...
    for idx, image_file in enumerate(image_list):
        ...
        if not os.path.exists(output_dir+img_name[:-4]):
            os.makedirs(output_dir+img_name[:-4])
        out_path = os.path.join(output_dir, img_name[:-4], img_name)
